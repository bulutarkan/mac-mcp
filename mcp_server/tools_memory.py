...
FEATURE_VECTOR_DIMS = embeddings.FEATURE_VECTOR_DIMS
FEATURE_VECTOR_BACKEND = embeddings.FEATURE_VECTOR_BACKEND
APPLE_VECTOR_BACKEND = embeddings.APPLE_VECTOR_BACKEND
MULTILINGUAL_VECTOR_BACKEND = embeddings.MULTILINGUAL_VECTOR_BACKEND
MULTILINGUAL_MODEL = embeddings.MULTILINGUAL_MODEL
MULTILINGUAL_DIMS = embeddings.MULTILINGUAL_DIMS
_FASTEMBED_WORKER_LOCK = embeddings.WORKER_LOCK
...
def _normalize(text: Any) -> str:
    return embeddings.normalize(text)


def _tokens(text: str) -> List[str]:
    return embeddings.tokens(text)


def _feature_vector(text: str) -> List[float]:
    return embeddings.feature_vector(text)


def _normalize_vector(vector: Sequence[float]) -> List[float]:
    return embeddings.normalize_vector(vector)


def _fastembed_cache(root: Path) -> Path:
    del root
    return embeddings.model_cache()


def _fastembed_idle_seconds() -> float:
    return embeddings.idle_seconds()


def _discard_fastembed_worker_locked(*, terminate: bool = False) -> None:
    embeddings.discard_worker_locked(terminate=terminate)


def _fastembed_worker_vectors(root: Path, texts: Sequence[str], *, allow_start: bool) -> Optional[List[List[float]]]:
    del root
    return embeddings.worker_vectors(texts, allow_start=allow_start)


def _multilingual_vectors(root: Path, texts: Sequence[str], *, allow_download: bool) -> Optional[List[List[float]]]:
    del root
    return embeddings.multilingual_vectors(texts, allow_start=allow_download)


def _apple_helper(root: Path) -> Optional[Path]:
    del root
    return embeddings.apple_helper()


def _apple_vectors(root: Path, texts: Sequence[str]) -> Optional[List[List[float]]]:
    del root
    return embeddings.apple_vectors(texts)


def _semantic_vectors(root: Path, texts: Sequence[str], *, allow_download: bool = False) -> Tuple[str, int, List[List[float]]]:
    # Keep this wrapper so existing tests/patches and memory behavior remain stable.
    mode = embeddings.embedding_mode()
