...
class RiskAndScopeTests(unittest.TestCase):
    def test_registry_covers_current_94_tool_surface(self) -> None:
        self.assertEqual(94, len(RISK_REGISTRY))
        for required in ("run_command", "browser_observe", "browser_do", "tool_discover", "tool_invoke", "spawn_agent", "mac_snapshot", "mac_act", "artifact_pipeline", "context_handoff", "browser_upload_artifact", "read_file", "lesson_search", "lesson_record", "lesson_feedback", "lesson_consolidate"):
